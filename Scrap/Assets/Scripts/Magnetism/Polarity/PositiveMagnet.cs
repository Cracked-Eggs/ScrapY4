...
public class PositiveMagnet : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        if (otherMagnet != null)
        {
            // If other magnet is positive, repel; if negative, attract
            if (otherMagnet.polarity == Magnet.Polarity.Positive)
            {
                // Repel (same polarity)
                Repel(other);
            }
            else if (otherMagnet.polarity == Magnet.Polarity.Negative)
            {
